...
// Which plan week contains a given ISO date, or null if it falls outside the
// block. Used to work out whose weekly review a freshly-saved run note affects.
export function resolveWeekForIsoDate(plan, isoDate) {
  if (!isoDate) return null;
  const year = Number(plan.meta.blockStart.slice(0, 4));
  const week = plan.weeks.find((w) => w.days.some((d) => toIsoDate(d.date, year) === isoDate));
